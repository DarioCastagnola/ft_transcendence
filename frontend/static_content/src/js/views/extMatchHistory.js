...
import { fetchMatchHistory, fetchMatchHistoryById, fetchNicknameByPlayerId, getStats } from "../service/apiService.js";

export default function extMatchHistory() {
    const html = `
   ...
   <div class="container">
	...
	setTimeout(() => {
		...
		async function prepareHistory() {
			const player_id = await getStats(Number(sessionStorage.getItem("clickedUserId")));
			const history = await fetchMatchHistoryById(Number(player_id.player)); // Recupera i dati
